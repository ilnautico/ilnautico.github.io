...
app.post("/generate-report", async (req, res) => {
  try {
    ...
    const email = "test@example.com"; // 仮（今は動作確認用）
    ...
    let parsed = {
      summary: "No fundamental incompatibility identified. Pilot required.",
      findings: "Thermal window must be validated.",
      conclusion: "Proceed with controlled pilot.",
    ...
    const data = {
      ...
      feasibility_class:
        parsed.feasibility === "HIGH"
          ? "level-high"
          : parsed.feasibility === "LOW"
          ? "level-low"
          : "level-moderate"
    };
